...
def plot_efficient_frontier_position(mu, cov, w_opt, rf, tickers, sleeve_of, A, sleeves):
    """
    Plot efficient frontier with current portfolio marked
    NOW RESPECTS SLEEVE CONSTRAINTS - portfolios stay within policy bounds
    """
    n_portfolios = 5000
    ...
    successful = 0
    attempts = 0
    max_attempts = n_portfolios * 10  # Allow more attempts to find valid portfolios
    
    print(f"Generating {n_portfolios} constrained portfolios for efficient frontier...")
    
    while successful < n_portfolios and attempts < max_attempts:
        attempts += 1
        
        # Generate weights that respect sleeve constraints
        weights = np.zeros(len(mu))
        
        # For each sleeve, allocate random weight within bounds
        for j, s in enumerate(sleeves):
            lb, ub = SLEEVE_BOUNDS[s]
            sleeve_weight = np.random.uniform(lb, ub)
            
            # Find tickers in this sleeve
            mask = A[j, :] > 0
            n_in_sleeve = mask.sum()
            
            if n_in_sleeve > 0:
                # Random allocation within sleeve (Dirichlet for better spread)
        ...
        # Validate constraints (should always pass with our generation method)
        valid = True
        for j, s in enumerate(sleeves):
            actual = float(A[j, :] @ weights)
            lb, ub = SLEEVE_BOUNDS[s]
            if not (lb - 1e-6 <= actual <= ub + 1e-6):
                valid = False
                break
        ...
        if valid:
            ...
            results[0, successful] = port_ret
            results[1, successful] = port_vol
            results[2, successful] = (port_ret - rf) / port_vol if port_vol > 0 else 0
            
            successful += 1
    
    # Trim results to successful portfolios
    results = results[:, :successful]
    print(f"✓ Generated {successful} valid portfolios (from {attempts} attempts)")
    ...
    plt.scatter(opt_vol, opt_ret, marker='*', color='red', s=500, 
               edgecolors='black', linewidth=2, label=f'Optimal (Sharpe={opt_sharpe:.3f})')
    
    plt.title('Efficient Frontier with Optimal Portfolio (Sleeve-Constrained)', 
...
def main():
    ...
    try:
        ...
        plot_efficient_frontier_position(mu, cov, w, rf, tickers, sleeve_of, A, sleeves)
